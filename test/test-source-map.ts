...
describe("sourceMap", () => {
    ...
    it("should emit correct source-map", () => {
        ...
        const bundle = fs.readFileSync(path.resolve("./build/test-source-map.test.js")).toString();
        const mapStr = fs.readFileSync(path.resolve("./build/test-source-map.test.js.map")).toString();
        const map = JSON.parse(mapStr);
        const consumer = new sourceMap.SourceMapConsumer(map);

        assert.ok(map.sources.indexOf("webpack:///./fixtures/source-map.ts") !== -1);
        ...
        const generatedTargetLine = searchLine("    return { name: name };", bundle);
